...
| --- | --- | --- |
| [Stable — v2026.9.13.1](https://github.com/DrMaks22/paperclip-localizations/releases/tag/v2026.9.13.1) | Official stable release **v2026.831.1**, commit `65ec059` | Default choice for that stable version. English and Russian UI. |
...
stable release v2026.9.13.1 for Paperclip v2026.831.1.
...
Corrections and new languages are welcome through pull requests to `main`. To add a language, create a raw nested catalog at `locales/<BCP47>.json`, such as `locales/de.json`, using [`locales/en.json`](locales/en.json) as the structural reference. In beta, project tooling infers the language's native display name; you do not need to edit the language switcher manually. The current stable release preserves its previously reviewed EN/RU integration; porting new languages to stable requires a separately verified release.
