...
[![Checks](https://github.com/DrMaks22/paperclip-localizations/actions/workflows/ci.yml/badge.svg)](https://github.com/DrMaks22/paperclip-localizations/actions/workflows/ci.yml) · [Releases](https://github.com/DrMaks22/paperclip-localizations/releases) · [Verification record](verification/v2026.9.12.1.md)
...
release v2026.9.12.1. My original Paperclip Git repository is at
/absolute/path/to/paperclip. Read and follow this release's instructions:
https://github.com/DrMaks22/paperclip-localizations/blob/v2026.9.12.1/docs/AGENT-INSTALL.md
...
This is a plain language task for the agent, not a product specific command. The [agent instructions](docs/AGENT-INSTALL.md) also cover an existing Paperclip installation.
...
Corrections and new languages are welcome through pull requests. To add a language, create a raw nested catalog at `locales/<BCP47>.json`, such as `locales/de.json`, using [`locales/en.json`](locales/en.json) as the structural reference. Project tooling infers the language's native display name; you do not need to edit the language switcher manually.
...
On Mondays, GitHub checks for upstream changes and a Codex agent prepares any necessary translation and patch updates in a separate pull request. The weekly Codex task is configured for 10:00 Europe/Moscow. Merging, publishing a stable release, and deployment require separate maintainer approval. See the [maintenance guide](docs/MAINTENANCE.md) for schedules, automation limits, and release checks.
